<?php

//$app->layout->leftMenu->addItem(['Main page', 'icon'=>'dashboard'],['main']);
$app->layout->leftMenu->addItem(['Main info','icon'=>'dashboard'],['index']);
$app->layout->leftMenu->addItem(['Users', 'icon'=>'users'],['users']);
$app->layout->leftMenu->addItem(['Folders', 'icon'=>'folder open outline'],['folders']);
$app->layout->leftMenu->addItem(['Files', 'icon'=>'file alternate outline'],['files']);
$app->layout->leftMenu->addItem(['Exit', 'icon'=>'sign out'],['logout_admin']);
